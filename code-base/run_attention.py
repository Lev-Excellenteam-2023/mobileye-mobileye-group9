...
def find_tfl_lights(c_image: np.ndarray, **kwargs) -> Dict[str, Any]:
    ...
    threshold = 0  # Adjust this threshold based on your image
    red_mask = (red_image > threshold) & (blue_image < red_image - 0.1) & (green_image < red_image - 0.1)
    # Threshold the correlation_sum to identify regions that closely match the kernel
    green_mask = (green_image > threshold) & (blue_image < green_image - 0.1) & (red_image < green_image - 0.1)
    red_coordinates = np.argwhere(red_mask)
    green_coordinates = np.argwhere(green_mask)
    # Extract x and y coordinates from the coordinates arrays
    x_red = red_coordinates[:, 1].tolist()
    y_red = red_coordinates[:, 0].tolist()
    x_green = green_coordinates[:, 1].tolist()
    y_green = green_coordinates[:, 0].tolist()
